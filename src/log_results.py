...
from src.data import match_signal
...
def create_results_df(args, data_loader, epoch):
    wandb_table = init_wandb_table()
    df = pd.DataFrame(columns=[RESULTS_DF_FILENAME, RESULTS_DF_LR_SNR, RESULTS_DF_PR_SNR, RESULTS_DF_PESQ,
    ...
    for i, data in enumerate(data_loader):
        ...
        add_data_to_wandb_table((hr, lr, pr), (pesq, stoi, lsd, sisnr, visqol, lr_snr, pr_snr), filename, args, wandb_table)
...
def init_wandb_table():
    columns = ['filename', 'hr audio', 'hr spectogram', 'lr audio', 'lr spectogram', 'pr audio',
               'pr spectogram', 'lr snr', 'pr snr', 'pesq', 'stoi', 'lsd', 'sisnr', 'visqol']
...
def add_data_to_wandb_table(signals, metrics, filename, args, wandb_table):
    ...
    pesq, stoi, lsd, sisnr, visqol, lr_snr, pr_snr = metrics
    ...
    wandb_table.add_data(filename, hr_wandb_audio, hr_wandb_spec, lr_wandb_audio, lr_wandb_spec,
                         ...
                         lr_snr, pr_snr, pesq, stoi, lsd, sisnr, visqol)
